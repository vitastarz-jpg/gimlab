...
# 1. 경로 설정 (HC 데이터셋용)
# =============================================================================
# [주의] MATLAB에서 생성한 마스크 파일들이 위치한 폴더 경로로 설정하세요.
BASE_DIR = r'D:\RUNo' 
...
def process_subject(sub_id):
    ...
    if not original_files:
        print(f"[Skip] No mask files found for {sub_id} in {BASE_DIR}")
        return
    ...
    for fpath in original_files:
        ...
        # 라벨(P번호) 추출 로직
        # 예: HC001_P36352_mask.nii -> P36352
        parts = fname.split('_')
        label = "Unknown"
        for p in parts:
    ...
    # -------------------------------------------------------------------------
    # STEP 2: Inspectro-Gadget 실행 (ROI 2개 자동 대응)
    # -------------------------------------------------------------------------
    sub_out_dir = os.path.join(INSP_OUTPUT_ROOT, sub_id)
    os.makedirs(sub_out_dir, exist_ok=True)
    
    # 2개면 2개, 3개면 3개 자동으로 인식하여 수행합니다.
...
# =============================================================================
# 3. 전체 실행 루프 (HC001 ~ HC029)
# =============================================================================
if __name__ == "__main__":
    print("Starting Batch Analysis for HC001 - HC029...")
    
    # HC001 ~ HC029 (range는 마지막 숫자 포함 안 함 -> 30)
    for i in range(1, 30):
        try:
            sub_id = f"HC{i:03d}"
